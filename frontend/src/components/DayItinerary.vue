  ...
  <div class="page-content--wide">
    <div v-if="loading" style="display:flex;flex-direction:column;gap:20px">
      ...
      <div style="display:flex;gap:8px">
        <div v-for="i in 5" :key="i" class="skeleton" style="width:58px;height:52px;border-radius:var(--radius-md)"></div>
      </div>
    ...
    <template v-else>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
        <div style="display:flex;align-items:center;gap:14px">
          ...
          <div>
            ...
            <h1 style="font:var(--fw-bold) 30px/1 var(--font-display);letter-spacing:-0.03em;display:flex;align-items:center;gap:8px">
      ...
      <div class="card" style="margin-bottom:24px;display:flex;gap:20px;flex-wrap:wrap;align-items:flex-start">
        <!-- Cities / places -->
        <div style="flex:1;min-width:200px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
            <i class="pi pi-map-marker" style="color:var(--brand);font-size:16px"></i>
            <span style="font:var(--fw-semibold) 14px/1 var(--font-sans);color:var(--text-strong)">Visiting</span>
          </div>
          <div v-if="!editingCity" style="display:flex;align-items:center;gap:8px;cursor:pointer" @click="editingCity = true; cityDraft = day?.city || ''">
            <span style="font:var(--fw-medium) 15px/1.2 var(--font-display);color:var(--text-strong)">
              {{ day?.city || 'Set cities...' }}
            </span>
          ...
          <div v-else style="display:flex;gap:6px;align-items:center">
            ...
            />
            <TfIconButton variant="ghost" size="sm" @click="saveCity"><i class="pi pi-check"></i></TfIconButton>
            <TfIconButton variant="ghost" size="sm" @click="editingCity = false"><i class="pi pi-times"></i></TfIconButton>
          </div>
        </div>
        ...
        <div style="flex:1;min-width:200px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
            ...
            <span style="font:var(--fw-semibold) 14px/1 var(--font-sans);color:var(--text-strong)">Overnight</span>
          </div>
          ...
          <div v-if="!editingOvernight">
            ...
            <template v-if="day?.overnightStay">
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <i class="pi pi-pencil" style="font-size:11px;color:var(--text-subtle);cursor:pointer" @click="editingOvernight = true; overnightDraft = day.overnightStay"></i>
              </div>
              <!-- Linked booking badge -->
              <div v-if="linkedBooking" style="margin-top:6px;display:flex;align-items:center;gap:6px">
                <TfBadge tone="accent" variant="soft">
                  <i class="pi pi-link" style="font-size:10px"></i>
                  {{ linkedBooking.name }}{{ linkedBooking.accommodationCity ? ' · ' + linkedBooking.accommodationCity : '' }}
                </TfBadge>
                <button
                  style="background:none;border:none;color:var(--text-subtle);font-size:11px;cursor:pointer;padding:2px"
                  @click="unlinkBooking"
            ...
            <template v-else>
              ...
              <div v-if="overnightSuggestion">
                <button
                  style="border:1.5px dashed var(--border-default);background:transparent;border-radius:var(--radius-md);padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:10px;color:var(--text-muted);font:var(--type-small);width:100%;transition:all var(--dur-fast) var(--ease-out);text-align:left"
                  @click="applyOvernightSuggestion"
                >
                  <i class="pi pi-sparkles" style="color:var(--gold-400);font-size:16px"></i>
                  <div>
                    <div style="font:var(--fw-semibold) 14px/1.2 var(--font-sans);color:var(--text-strong)">{{ overnightSuggestion.name }}</div>
                    <div style="font:var(--fw-regular) 12px/1.2 var(--font-sans);color:var(--text-muted);margin-top:2px">{{ overnightSuggestion.accommodationCity }} · from booking</div>
                  </div>
                </button>
                <div style="margin-top:6px;font:var(--type-small);color:var(--text-subtle);cursor:pointer" @click="editingOvernight = true; overnightDraft = ''">
                  or enter manually...
                </div>
              </div>
              <!-- No suggestion -->
              <div v-else style="display:flex;align-items:center;gap:8px;cursor:pointer" @click="editingOvernight = true; overnightDraft = ''">
                <span style="font:var(--fw-regular) 14px/1.2 var(--font-sans);color:var(--text-subtle)">Not set — click to add</span>
              </div>
            </template>
          </div>
          ...
          <div v-else style="display:flex;gap:6px;align-items:center">
            <PInputText
              ...
              autofocus
            />
            <TfIconButton variant="ghost" size="sm" @click="saveOvernightManual"><i class="pi pi-check"></i></TfIconButton>
            <TfIconButton variant="ghost" size="sm" @click="editingOvernight = false"><i class="pi pi-times"></i></TfIconButton>
          </div>
        </div>
      </div>
            ...
            <TfCard interactive @click="startEdit(a)" style="cursor:pointer">
              <div style="display:flex;align-items:center;gap:12px">
                ...
                <div style="flex:1;min-width:0">
                  <div style="display:flex;align-items:center;gap:8px">
                    <span style="font:var(--fw-semibold) 16px/1.2 var(--font-sans);color:var(--text-strong)">{{ a.name }}</span>
                    <TfBadge v-if="a.type" tone="neutral" variant="soft">{{ typeLabel(a.type) }}</TfBadge>
                  </div>
                  <div style="font:var(--fw-regular) 13px/1.3 var(--font-sans);color:var(--text-muted);margin-top:3px;display:flex;gap:10px;flex-wrap:wrap">
                    <span v-if="a.startTime" style="display:inline-flex;align-items:center;gap:4px">
                      <i class="pi pi-clock" style="font-size:12px"></i>
                      {{ a.startTime?.slice(0,5) }}{{ a.endTime ? ' – ' + a.endTime.slice(0,5) : '' }}
                    </span>
                    <span v-if="a.address" style="display:inline-flex;align-items:center;gap:4px">
                      <i class="pi pi-map-marker" style="font-size:12px"></i>
                      {{ a.address }}
                    </span>
                    <span v-if="a.placeName" style="display:inline-flex;align-items:center;gap:4px;color:var(--accent)">
                  ...
                  <div v-if="a.notes" style="font:var(--type-small);color:var(--text-muted);margin-top:4px;font-style:italic">{{ a.notes }}</div>
                </div>
      ...
      <template v-else>
        ...
        <div v-if="cityPlaces.length" style="margin-top:18px">
          <div class="addfrom-title"><i class="pi pi-map-marker" style="color:var(--brand)"></i> Add from places · {{ day?.city }}</div>
          <div class="addfrom-grid">
            <button v-for="p in cityPlaces" :key="p.id" type="button" class="addfrom-item" @click="openAddFromPlace(p)">
              <span v-if="p.photos && p.photos.length" class="addfrom-thumb" :style="{ backgroundImage: `url(${p.photos[0]})` }"></span>
              <span v-else class="cat-icon cat-icon--lg" :style="placeTypeStyle(p.type)">{{ placeTypeEmoji(p.type) }}</span>
              <span style="flex:1;min-width:0">
                <span class="addfrom-name">{{ p.name }}</span>
                <span class="addfrom-sub">{{ [placeTypeLabel(p.type), p.city || p.address].filter(Boolean).join(' · ') }}</span>
              </span>
              <i class="pi pi-plus" style="color:var(--brand);font-size:16px"></i>
            </button>
          </div>
          <div style="text-align:center;margin-top:16px">
            <TfButton variant="ghost" @click="openAddDialog"><i class="pi pi-plus" style="font-size:13px"></i> Or add manually</TfButton>
          </div>
        </div>
        <div v-else style="text-align:center;margin-top:16px">
          <TfButton variant="primary" @click="openAddDialog"><i class="pi pi-plus" style="font-size:13px"></i> Add activity</TfButton>
        </div>
      </template>

      <!-- Day total by category -->
      <div v-if="activities.length" class="card" style="margin-top:18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
        <div style="display:flex;gap:18px;flex-wrap:wrap">
          <div v-for="(amount, type) in costByType" :key="type" style="display:flex;align-items:center;gap:7px">
            <div class="cat-icon cat-icon--sm" :style="catStyle(type)">{{ typeIcon(type) }}</div>
            <span style="font:var(--fw-medium) 13px/1 var(--font-sans);color:var(--text-muted)">{{ typeLabel(type) }}</span>
            <span class="money money--sm">{{ amount.toFixed(2) }} {{ currency }}</span>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:10px">
          <span style="font:var(--fw-medium) 13px/1 var(--font-sans);color:var(--text-muted)">Day total</span>
      ...
      <aside class="itin-map">
        <div class="itin-map-head">
          ...
          <span class="text-subtle text-sm">{{ activityMarkers.length }} point{{ activityMarkers.length === 1 ? '' : 's' }}</span>
        </div>
    ...
    <!-- Activity Drawer -->
    <TfDrawer v-model="showDrawer" :title="editingActivity ? 'Edit activity' : 'New activity'" :eyebrow="day ? `Day ${day.dayNumber} · ${day.city || ''}` : ''">
      <form @submit.prevent="saveActivity" style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="field" style="gap:8px">
          ...
          <div class="src-choice">
            <button type="button" class="src-btn" :class="{ 'src-btn--on': actSource === 'search' }" @click="setActSource('search')">
              <i class="pi pi-search"></i><span>Search</span>
            </button>
            <button type="button" class="src-btn" :class="{ 'src-btn--on': actSource === 'import' }" @click="setActSource('import')">
              <i class="pi pi-download"></i><span>Import</span>
            </button>
            <button type="button" class="src-btn" :class="{ 'src-btn--on': actSource === 'manual' }" @click="setActSource('manual')">
        ...
        <div v-if="actSource === 'search'" class="src-panel">
          <template v-if="!form.placeId">
            <div class="field">
              ...
              <PSelect :modelValue="form.placeId" @update:modelValue="onPlacePicked"
                       :options="placeOptions" optionLabel="label" optionValue="value"
                       filter showClear placeholder="Pick a saved place…" class="w-full" />
            </div>
            <div class="field">
              <label>…or find a new place <span v-if="findingPlace" class="text-muted" style="font-weight:400;font-size:12px">· saving…</span></label>
              <TfPlaceSearch placeholder="Search a beach, restaurant, landmark…" @select="onActivityGeoPicked" />
            </div>
          </template>
          <div v-else class="linked-place">
            <i class="pi pi-bookmark" style="color:var(--accent)"></i>
            <span class="linked-name">{{ linkedPlaceName }}</span>
            <button type="button" class="link-edit" @click="goEditPlace">Edit place <i class="pi pi-arrow-up-right" style="font-size:10px"></i></button>
            <button type="button" class="del-btn" @click="unlinkPlace" v-tooltip="'Unlink'"><i class="pi pi-times"></i></button>
          </div>
        </div>
        ...
        <div v-else-if="actSource === 'import'" class="src-panel">
          <template v-if="!form.placeId">
            <div class="field">
              ...
              <div style="display:flex;gap:8px">
                <PInputText v-model="activityImportUrl" placeholder="https://maps.app.goo.gl/…" class="w-full" @keyup.enter="runActivityImport" />
                <TfButton variant="soft" @click="runActivityImport" :disabled="findingPlace || !activityImportUrl">
                  <i class="pi pi-download" :style="findingPlace ? 'animation:spin 1s linear infinite' : ''"></i>
                </TfButton>
              </div>
        ...
        <div v-else-if="actSource === 'manual'" class="src-panel">
          ...
          <div class="field">
            <label>Type</label>
            <PSelect v-model="form.type" :options="typeOptions" optionLabel="label" optionValue="value" placeholder="Select type" class="w-full" />
          </div>
          <div class="field">
            <label>Address</label>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="field">
              <label>Latitude <span style="font-weight:400;font-size:12px;color:var(--text-muted)">(optional)</span></label>
              <PInputNumber v-model="form.latitude" :maxFractionDigits="7" placeholder="35.0116" class="w-full" />
            </div>
            <div class="field">
              <label>Longitude <span style="font-weight:400;font-size:12px;color:var(--text-muted)">(optional)</span></label>
              <PInputNumber v-model="form.longitude" :maxFractionDigits="7" placeholder="135.7681" class="w-full" />
            </div>
          </div>
          <label class="save-place-toggle">
            <input type="checkbox" v-model="saveToPlaces" />
            <span><i class="pi pi-bookmark" style="font-size:13px"></i> Save as a place (so it shows on the map)</span>
          </label>
        </div>
        ...
        <template v-if="showCommon">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="field">
              <label>Start time <span style="font-weight:400;font-size:12px;color:var(--text-muted)">(optional)</span></label>
              <PInputText v-model="form.startTime" placeholder="09:00" class="w-full" />
            </div>
            <div class="field">
              <label>End time <span style="font-weight:400;font-size:12px;color:var(--text-muted)">(optional)</span></label>
              <PInputText v-model="form.endTime" placeholder="11:00" class="w-full" />
            </div>
          </div>
          <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px">
            <div class="field">
              <label>Cost <span style="font-weight:400;font-size:12px;color:var(--text-muted)">(optional)</span></label>
              <PInputNumber v-model="form.costEstimate" placeholder="0.00" :minFractionDigits="2" class="w-full" />
            </div>
            <div class="field">
              <label>Currency</label>
              <PSelect v-model="form.costCurrency" :options="currencyOptions" editable class="w-full" />
            </div>
          </div>
          <div class="field">
            <label>Notes</label>
      ...
      <template #footer>
        ...
        <TfButton v-if="editingActivity" variant="ghost" @click="confirmDelete(editingActivity)">Delete</TfButton>
      </template>
    </TfDrawer>
...
import { TfButton, TfIconButton, TfBadge, TfCard, TfDrawer, TfCitySearch, TfPlaceSearch } from './ui';
...
const emptyForm = { name: '', type: null, startTime: '', endTime: '', address: '', costEstimate: null, costCurrency: null, notes: '', placeId: null, latitude: null, longitude: null };
...
const unlinkPlace = () => { form.value.placeId = null; };
const goEditPlace = () => {
...
const showCommon = computed(() =>
  actSource.value === 'manual' || !!form.value.placeId || !!(form.value.name && String(form.value.name).trim())
);
const setActSource = (s) => { actSource.value = s; };
...
const loadPlacesLib = async () => {
  try { placesLib.value = (await api.get('/api/places')).data || []; } catch { /* non-fatal */ }
};
...
const PLACE_TO_ACT = {
  BEACH: 'BEACH', NATURE: 'NATURE', PARK: 'NATURE', RESTAURANT: 'RESTAURANT', SHOP: 'SHOPPING',
  MUSEUM: 'SIGHTSEEING', SIGHTSEEING: 'SIGHTSEEING', VIEWPOINT: 'SIGHTSEEING',
  NEIGHBORHOOD: 'NEIGHBORHOOD', PORT: 'TRANSPORT', AIRPORT: 'TRANSPORT', OTHER: 'OTHER',
};
...
const openAddFromPlace = (p) => {
  ...
  form.value = { ...emptyForm, placeId: p.id, name: p.name, address: p.address || '', type: placeToActivityType(p.type), costCurrency: accountCurrency.value };
...
const ACT_TO_PLACE_TYPE = {
  SIGHTSEEING: 'SIGHTSEEING', BEACH: 'BEACH', NATURE: 'NATURE', NEIGHBORHOOD: 'NEIGHBORHOOD',
  RESTAURANT: 'RESTAURANT', MEAL_STOP: 'RESTAURANT', SHOPPING: 'SHOP', TRANSPORT: 'OTHER', OTHER: 'OTHER',
};
...
const onActivityGeoPicked = async (geo) => {
  ...
  try {
    const res = await api.post('/api/places/geocode', { text: geo.displayName || geo.name, country: null });
  ...
  } finally { findingPlace.value = false; }
};
...
const typeIcon = (t) => ({
  SIGHTSEEING: '\u{1F3DB}', BEACH: '\u{1F3D6}', NATURE: '\u{1F33F}', NEIGHBORHOOD: '\u{1F3D8}',
  RESTAURANT: '\u{1F37D}', MEAL_STOP: '\u2615', SHOPPING: '\u{1F6CD}', TRANSPORT: '\u{1F68C}', OTHER: '\u{1F4CC}'
}[t] ?? '\u{1F4CC}');
...
const dayTotal = computed(() => activities.value.reduce((s, a) => s + (Number(a.costEstimate) || 0), 0));
...
const activityMarkers = computed(() =>
  activities.value
    ...
    .map(a => ({ lat: Number(a.placeLatitude), lon: Number(a.placeLongitude), label: a.placeName || a.name }))
);
// activity id -> its number on the map (same order as the markers)
const stopNumbers = computed(() => {
  const map = {}; let n = 0;
  activities.value.forEach(a => { if (a.placeLatitude != null && a.placeLongitude != null) map[a.id] = ++n; });
  return map;
});
...
const cityPlaces = computed(() => {
  ...
  if (!c) return [];
  return placesLib.value.filter(p => {
...
const overnightSuggestion = computed(() => {
  ...
  return bookings.value.find(b =>
    b.category === 'ACCOMMODATION' && b.checkIn && b.checkOut &&
    dayDate >= b.checkIn && dayDate < b.checkOut
  ) || null;
});
...
const saveOvernightManual = async () => {
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to update overnight', life: 3000 });
  }
};
...
const applyOvernightSuggestion = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to link booking', life: 3000 });
  }
};
  ...
  try {
    ...
    if (saveToPlaces.value && !form.value.placeId && form.value.name) {
      try {
        ...
        toast.add({ severity: 'warn', summary: 'Note', detail: 'Activity saved, but adding to places failed', life: 3000 });
      }
    }
    ...
    if (editingActivity.value) {
      ...
      toast.add({ severity: 'success', summary: 'Updated', detail: 'Activity updated', life: 3000 });
    } else {
      const res = await api.post(`/api/days/${dayId.value}/activities`, payload);
      activities.value.push(res.data);
      toast.add({ severity: 'success', summary: 'Added', detail: `"${res.data.name}" added`, life: 3000 });
    }
    showDrawer.value = false;
  } catch {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to save activity', life: 3000 });
  } finally {
    saving.value = false;
  }
};
...
.itin-map { position: sticky; top: 0; }
.itin-map-head {
  display: flex;
  align-items: center;
...
.addfrom-item:hover { border-color: var(--border-default); box-shadow: var(--shadow-sm); }
.addfrom-thumb {
  flex: none;
...
@media (max-width: 1024px) {
  ...
  .itin-map { position: static; }
  .itin-map-empty { height: 280px; }
}
...
.src-btn i { font-size: 18px; }
.src-btn:hover { border-color: var(--border-default); color: var(--text-strong); }
.src-btn--on { border-color: var(--brand); background: var(--brand-soft); color: var(--brand-pressed); }
...
.save-place-toggle input { accent-color: var(--brand); width: 18px; height: 18px; }
.save-place-toggle span { display: inline-flex; align-items: center; gap: 6px; }
</style>
