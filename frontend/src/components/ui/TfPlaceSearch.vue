<template>
  <div class="tf-place-search" ref="wrapper">
    <div style="position:relative">
      <i class="pi pi-map-marker" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--text-subtle);font-size:13px;pointer-events:none"></i>
      <input
        :value="displayValue"
        @input="onInput"
        @focus="onFocus"
      ...
      />
      <i v-if="searching" class="pi pi-spin pi-spinner" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);color:var(--text-subtle);font-size:13px"></i>
      <button
    ...
    <div v-if="open && (results.length || noResults || searching)" class="tf-place-dropdown">
      <div v-if="searching && !results.length" style="padding:14px;text-align:center;font:var(--type-small);color:var(--text-muted)">
        Searching...
      </div>
      <div
        v-for="r in results"
      ...
      <div v-if="noResults" style="padding:14px;text-align:center;font:var(--type-small);color:var(--text-subtle)">
...
const onInput = (e) => {
  ...
  clearTimeout(timer);
  if (!val || val.length < 2) { results.value = []; open.value = false; return; }
  open.value = true;
...
const typeEmoji = (t) => ({
  hotel: '🏨', airport: '✈️', city: '🌆', tourism: '🎭',
  food: '🍽️', amenity: '📍', station: '🚉', place: '📍',
}[t] || '📍');

const typeBadge = (t) => ({
  hotel: 'Hotel', airport: 'Airport', city: 'City', tourism: 'Attraction',
  food: 'Food', amenity: 'Place', station: 'Station', place: 'Place',
}[t] || '');
...
.tf-place-input::placeholder { color: var(--text-subtle); }
...
.tf-place-option:hover { background: var(--surface-sunken); }
.tf-place-option:not(:last-child) { border-bottom: 1px solid var(--border-subtle); }
