<template>
  <div class="page-content">
    ...
    <template v-else-if="trip">
      ...
      <div class="page-head">
        <div>
          ...
          <h1>{{ trip.title }}</h1>
          <p>{{ trip.destination || '' }}{{ trip.destination && trip.startDate ? ' · ' : '' }}{{ formatDateRange(trip.startDate, trip.endDate) }}</p>
        </div>
      ...
      <div style="display:grid;grid-template-columns:1.4fr 1fr;gap:20px;margin-bottom:20px">
        ...
        <div class="card">
          ...
          <div v-if="budgetData" style="display:flex;gap:26px;margin-bottom:18px">
            <div>
              <div style="font:400 12px/1 var(--font-sans);color:var(--text-muted);margin-bottom:6px">Planned</div>
              <span class="money money--lg">{{ fmtMoney(budgetData.totalPlanned) }}</span>
            </div>
            <div>
              <div style="font:400 12px/1 var(--font-sans);color:var(--text-muted);margin-bottom:6px">Paid</div>
              <span class="money money--lg" style="color:var(--success-500)">{{ fmtMoney(Number(budgetData.bookingsPaid || 0) + Number(budgetData.expensesTotal || 0)) }}</span>
            </div>
            <div>
              <div style="font:400 12px/1 var(--font-sans);color:var(--text-muted);margin-bottom:6px">Remaining</div>
              <span class="money money--lg" style="color:var(--coral-600)">{{ fmtMoney(budgetData.bookingsRemaining) }}</span>
            </div>
          </div>
          <template v-if="budgetData && Number(budgetData.totalPlanned) > 0">
            <div class="tf-progress">
              <div class="tf-progress-fill" :style="{ width: Math.min(100, Math.round(((Number(budgetData.bookingsPaid || 0) + Number(budgetData.expensesTotal || 0)) / Number(budgetData.totalPlanned)) * 100)) + '%' }"></div>
            </div>
            <div style="font:var(--fw-medium) 12px/1 var(--font-mono);color:var(--text-muted);margin-top:8px">
              {{ Math.round(((Number(budgetData.bookingsPaid || 0) + Number(budgetData.expensesTotal || 0)) / Number(budgetData.totalPlanned)) * 100) }}% paid
            </div>
          </template>
          <div v-else-if="!budgetData" style="font:var(--type-small);color:var(--text-subtle)">
            <TfBadge :tone="statusTone(trip.status)" variant="solid">{{ statusLabel(trip.status) }}</TfBadge>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div v-if="upcomingPayments.length" style="display:flex;flex-direction:column;gap:10px">
            <div v-for="p in upcomingPayments" :key="p.paymentId" style="display:flex;align-items:center;gap:11px">
              <div class="cat-icon cat-icon--sm" :style="catIconStyle(p.category)">{{ catIcon(p.category) }}</div>
              <div style="flex:1;min-width:0">
                ...
                <div style="font:var(--fw-medium) 11px/1 var(--font-mono);color:var(--text-muted);margin-top:2px">{{ formatDateShort(p.dueDate) }}</div>
              </div>
              <span class="money money--sm">{{ Number(p.amount).toFixed(2) }} {{ budgetData?.baseCurrency || accountCurrency.value }}</span>
            </div>
          </div>
          <div v-else style="font:var(--type-small);color:var(--text-subtle)">No upcoming payments</div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:20px">
        ...
        <div style="display:flex;gap:24px;flex-wrap:wrap">
          <div style="display:flex;align-items:center;gap:11px">
            ...
            <div>
              <div style="font:var(--fw-semibold) 14px/1.2 var(--font-sans);color:var(--text-strong)">{{ trip.destination || 'Not set' }}</div>
              <div style="font:var(--fw-medium) 11px/1 var(--font-mono);color:var(--text-muted);margin-top:2px">Destination</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:11px">
            <i class="pi pi-calendar" style="color:var(--brand);font-size:16px"></i>
            <div>
              <div style="font:var(--fw-semibold) 14px/1.2 var(--font-sans);color:var(--text-strong)">{{ formatDate(trip.startDate) }} – {{ formatDate(trip.endDate) }}</div>
              <div style="font:var(--fw-medium) 11px/1 var(--font-mono);color:var(--text-muted);margin-top:2px">Dates</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="daysLoading" style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px">
        <div v-for="i in 4" :key="i" class="skeleton" style="height:100px"></div>
      </div>

      <div v-else-if="days.length" class="dashboard-grid dashboard-grid--4col" style="margin-bottom:24px">
        <TfCard
          v-for="day in days"
          :key="day.id"
          interactive
        ...
        >
          <div style="display:flex;justify-content:space-between;align-items:baseline">
            <span style="font:var(--fw-bold) 15px/1 var(--font-display);color:var(--text-strong)">Day {{ day.dayNumber }}</span>
            <span style="font:var(--fw-medium) 11px/1 var(--font-mono);color:var(--text-subtle)">{{ formatDateShort(day.date) }}</span>
          </div>
          <div
          ...
          >
            <PInputText
              ...
              autofocus
            />
            <TfIconButton variant="ghost" size="sm" @click="saveDayCity(day.id)"><i class="pi pi-check"></i></TfIconButton>
          </div>
          <div
            v-else
            style="font:var(--fw-medium) 13px/1.2 var(--font-sans);color:var(--accent);margin:8px 0 0;cursor:pointer"
      ...
      <div v-else-if="!daysLoading && !(trip.startDate && trip.endDate)" class="empty-state" style="margin-bottom:24px">
      ...
      <div class="dashboard-grid dashboard-grid--2col">
        <TfCard interactive @click="$router.push(`/trips/${tripId}/bookings`)" style="cursor:pointer">
          <div style="display:flex;align-items:center;gap:12px">
            <div class="cat-icon cat-icon--lg" style="background:var(--teal-50);color:var(--accent)">
              <i class="pi pi-ticket"></i>
            </div>
            <div style="flex:1">
              <div style="font:var(--fw-bold) 17px/1.15 var(--font-display);color:var(--text-strong)">Bookings</div>
              <div style="font:var(--type-small);color:var(--text-muted);margin-top:2px">Flights, hotels, activities</div>
            </div>
            <i class="pi pi-chevron-right" style="color:var(--text-subtle)"></i>
          </div>
        </TfCard>
        <TfCard interactive @click="$router.push(`/trips/${tripId}/budget`)" style="cursor:pointer">
          <div style="display:flex;align-items:center;gap:12px">
            <div class="cat-icon cat-icon--lg" style="background:var(--gold-50);color:var(--gold-400)">
              <i class="pi pi-dollar"></i>
            </div>
            <div style="flex:1">
              <div style="font:var(--fw-bold) 17px/1.15 var(--font-display);color:var(--text-strong)">Budget</div>
              <div style="font:var(--type-small);color:var(--text-muted);margin-top:2px">Plan vs actual spending</div>
            </div>
            <i class="pi pi-chevron-right" style="color:var(--text-subtle)"></i>
          </div>
        </TfCard>
      ...
      <form @submit.prevent="saveEdit" style="display:flex;flex-direction:column;gap:16px">
        ...
        <div class="field">
          ...
          <PDatePicker
            ...
            :manualInput="false"
          />
          <small v-if="days.length" style="color:var(--text-muted);margin-top:6px;display:block">
    ...
    <PDialog v-model:visible="rescheduleConfirm" modal header="Move trip dates?" :style="{ width: '440px' }">
      <div v-if="rescheduleImpact" style="display:flex;flex-direction:column;gap:12px">
        <p style="margin:0;color:var(--text-body)">
          New dates: <strong>{{ formatDate(pendingEdit?.startDate) }} – {{ formatDate(pendingEdit?.endDate) }}</strong>
        </p>
        <ul style="margin:0;padding-left:18px;display:flex;flex-direction:column;gap:6px;color:var(--text-body)">
          <li v-if="rescheduleImpact.delta !== 0">
            The itinerary shifts <strong>{{ Math.abs(rescheduleImpact.delta) }}
            day{{ Math.abs(rescheduleImpact.delta) === 1 ? '' : 's' }}
            {{ rescheduleImpact.delta > 0 ? 'later' : 'earlier' }}</strong> — day order and plans are kept.
          </li>
          ...
          <li v-if="rescheduleImpact.removed > 0" style="color:var(--coral-600)">
            <strong>{{ rescheduleImpact.removed }}
            day{{ rescheduleImpact.removed === 1 ? '' : 's' }}</strong> now outside the range
            will be deleted, together with their activities.
          </li>
          <li v-if="rescheduleImpact.added > 0">
            <strong>{{ rescheduleImpact.added }}
            empty day{{ rescheduleImpact.added === 1 ? '' : 's' }}</strong> will be added to fill the new range.
          </li>
        </ul>
      </div>
      <template #footer>
...
import { TfButton, TfBadge, TfCard, TfIconButton, TfDrawer, TfCitySearch } from './ui';
...
const statusLabel = (s) => ({ DRAFT: 'Draft', PLANNED: 'Planned', ACTIVE: 'Active', COMPLETED: 'Completed' }[s]);
const statusTone = (s) => ({ DRAFT: 'neutral', PLANNED: 'gold', ACTIVE: 'brand', COMPLETED: 'success' }[s] || 'neutral');

const upcomingPayments = computed(() => (budgetData.value?.upcomingPayments || []).slice(0, 3));
const fmtMoney = (v) => Number(v || 0).toFixed(2);
const catIcon = (c) => ({ TRANSPORTATION: '\u{1F68C}', ACCOMMODATION: '\u{1F3E8}', ACTIVITY: '\u{1F3AB}' }[c] ?? '\u{1F4CC}');
...
const startEdit = () => {
  ...
  editDateRange.value = trip.value.startDate && trip.value.endDate
    ...
    d.days.forEach(day => {
      ...
      if (day.activities.length) {
        day.activities.forEach(a => {
          ...
          if (a.startTime) html += ` <span class="muted">${a.startTime.slice(0,5)}${a.endTime ? '–' + a.endTime.slice(0,5) : ''}</span>`;
          if (a.address) html += ` <span class="muted">@ ${a.address}</span>`;
          if (a.costEstimate) html += ` <span class="muted">${a.costEstimate} ${accountCurrency.value}</span>`;
          if (a.notes) html += `<br><span class="muted">${a.notes}</span>`;
          html += `</div>`;
        });
      } else { html += `<p class="muted">No activities</p>`; }
    });
...
const generateDays = async () => {
  ...
  try {
    ...
    days.value = res.data;
    toast.add({ severity: 'success', summary: 'Generated', detail: `${res.data.length} days created`, life: 3000 });
  } catch {
    toast.add({ severity: 'error', summary: 'Error', detail: 'Failed to generate days', life: 3000 });
  } finally {
...
onMounted(async () => {
  try {
    ...
    await fetchDays();
    try { budgetData.value = (await api.get(`/api/trips/${tripId}/budget`)).data; } catch { /* no budget yet */ }
  } catch {
