<template>
  <div class="page-content">
    ...
    <!-- Loading skeleton -->
    <div v-if="store.loading" style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px">
      <div v-for="i in 3" :key="i" style="background:var(--surface-card);border:1px solid var(--border-subtle);border-radius:var(--radius-lg);overflow:hidden">
    ...
    <div v-if="!filteredTrips.length && !store.loading" class="empty-state">
      ...
      <p>{{ filterStatus === 'ALL' ? 'Add your first trip and start planning.' : 'No trips with this status.' }}</p>
      <PButton v-if="filterStatus === 'ALL'" icon="pi pi-plus" label="New trip" @click="showDialog = true" />
    </div>
    ...
    >
      <form @submit.prevent="addTrip" class="dialog-form">
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:11px 14px;background:var(--info-100);border-radius:var(--radius-md);color:var(--info-500);font:var(--type-small)">
          <i class="pi pi-info-circle"></i> Days will be generated automatically from your selected dates.
        </div>
...
import { TfBadge, TfCitySearch } from './ui';
...
const addTrip = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Created', detail: `Trip "${trip.title}" added`, life: 3000 });
  } catch {
