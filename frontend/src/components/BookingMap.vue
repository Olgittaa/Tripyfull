<template>
  <div ref="el" class="booking-map" :class="{ 'booking-map--hidden': !hasMarkers }" :style="height ? { height: height + 'px' } : null"></div>
</template>
...
function renderMarkers() {
  ...
  if (valid.length >= 2) {
    L.polyline(valid.map(m => [m.lat, m.lon]), props.numbered
      ? { color: '#0e5c55', weight: 2.4, dashArray: '6 5', opacity: 0.85 }
      : { color: '#6366f1', weight: 2.5, dashArray: '7 5', opacity: 0.7 }).addTo(map);
    map.fitBounds(valid.map(m => [m.lat, m.lon]), { padding: [40, 40] });
  } else {
...
watch(() => props.markers, async (newVal) => {
