    ...
    <template v-else>
      ...
      <div class="card" style="margin-bottom:20px">
        ...
        <div class="field">
          ...
          <p style="font:var(--type-small);color:var(--text-muted);margin:0 0 6px">All prices will be converted to this currency for totals.</p>
          <PSelect
      ...
      <div class="card" style="margin-bottom:20px">
        ...
        <div class="field">
          ...
          <p style="font:var(--type-small);color:var(--text-muted);margin:0 0 6px">Optional. Used for suggestions and defaults.</p>
          <PSelect
            v-model="form.region"
      ...
      <div class="card" style="margin-bottom:20px">
        ...
        <div style="margin-top:12px;padding:10px 14px;background:var(--surface-sunken);border-radius:var(--radius-md);font:var(--type-small);color:var(--text-muted)">
          Preview: <strong style="color:var(--text-strong)">{{ datePreview }}</strong> · <strong style="color:var(--text-strong)">{{ timePreview }}</strong>
        </div>
      </div>
...
import { prefs, baseCurrency, updateSettings } from '../auth.js';
...
const datePreview = computed(() => {
  ...
  switch (form.value.dateFormat) {
    case 'MM/DD/YYYY': return `${mon}/${String(day).padStart(2,'0')}/${yr}`;
    case 'YYYY-MM-DD': return `${yr}-${mon}-${String(day).padStart(2,'0')}`;
    case 'DD.MM.YYYY': return `${String(day).padStart(2,'0')}.${mon}.${yr}`;
    case 'D MMM YYYY': return `${day} ${months[d.getMonth()]} ${yr}`;
    default: return `${String(day).padStart(2,'0')}/${mon}/${yr}`;
  }
});
