      ...
      <template v-if="currentTrip">
        <div class="sidebar-trip-info">
          ...
          <div class="sidebar-trip-meta">
            {{ currentTrip.destination || '' }}{{ currentTrip.destination && currentTrip.startDate ? ' · ' : '' }}{{ currentTrip.startDate ? formatDateRange(currentTrip.startDate, currentTrip.endDate) : '' }}
          </div>
        </div>
      ...
      <div class="sidebar-user">
        ...
        <div style="min-width:0;line-height:1.25;flex:1">
          ...
          <div style="display:flex;gap:8px;align-items:center;margin-top:2px">
            <router-link to="/settings" class="sidebar-user-sub" style="text-decoration:none">Settings</router-link>
            <span style="color:var(--border-default)">·</span>
            <span class="sidebar-user-sub" @click="logout">Sign out</span>
          </div>
        </div>
        <span style="font:var(--fw-medium) 11px/1 var(--font-mono);color:var(--text-subtle);background:var(--surface-sunken);padding:3px 8px;border-radius:var(--radius-pill)">{{ baseCurrency }}</span>
      </div>
    </aside>
...
import { username, baseCurrency, clearAuth } from './auth.js';
import { useTripStore } from './stores/tripStore.js';
import { TfAvatar } from './components/ui';
...
const goHome = () => { lastTripId.value = null; sidebarTrip.value = null; router.push('/trips'); };
...
watch(() => route.params.id || route.params.tripId, (id) => {
...
watch(activeTripId, async (tripId) => {
